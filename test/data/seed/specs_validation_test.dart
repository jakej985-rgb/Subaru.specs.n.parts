  ...
  group('Spec Data Validation', () {
    ...
    setUpAll(() {
      ...
      expect(
        file.existsSync(),
        isTrue,
        reason: 'Missing specs.json at: $filePath',
      );
      ...
      for (final spec in specs) {
        ...
        if (fluidCategories.contains(category) && title.contains('Capacity')) {
          expect(
            ...
            isTrue,
            reason:
                'Invalid Fluid Format for ID ${spec['id']} ($title): "$body". Expected "X.X Liters (Y.Y Quarts)..."',
          );
        }
      }
    });
      ...
      for (final spec in specs) {
        ...
        if (category == 'Torque') {
          expect(
            ...
            isTrue,
            reason:
                'Invalid Torque Format for ID ${spec['id']} ($title): "$body". Expected "X.X ft-lbs (Y.Y Nm)..."',
          );
        }
      }
    });
    ...
    test('Key platforms must have Oil Capacity specs', () {
      ...
      expect(
        hasGd,
        isTrue,
        reason: 'Missing Oil Capacity spec for GD platform',
      );
      expect(
        hasVa,
        isTrue,
        reason: 'Missing Oil Capacity spec for VA platform',
      );
      expect(
        hasVb,
        isTrue,
        reason: 'Missing Oil Capacity spec for VB platform',
      );
    });
    ...
    test('Key platforms must have Bolt Pattern specs', () {
      ...
      for (final spec in specs) {
        ...
        if (tags.contains('bolt_pattern')) {
          ...
          if (tags.contains('vb') && tags.contains('wrx')) {
            hasVb = true;
          }
      ...
      expect(
        hasGdSti04,
        isTrue,
        reason: 'Missing 2004 STI specific Bolt Pattern (5x100)',
      );
    });
    ...
    test('Specs with "Capacity" in title must have "capacity" tag', () {
      for (final spec in specs) {
        ...
        if (title.toLowerCase().contains('capacity')) {
          expect(
            tags.contains('capacity'),
            isTrue,
            reason:
                'Spec "${spec['id']}" with title "$title" is missing "capacity" tag',
          );
        }
      }
    });
    ...
    test('Platforms with Oil Capacity must have Oil Viscosity', () {
      ...
      for (final platform in platformsWithCapacity) {
        expect(
          platformsWithViscosity.contains(platform),
          isTrue,
          reason:
              'Platform $platform has Oil Capacity but is missing Oil Viscosity spec',
        );
      }
    });
  });
