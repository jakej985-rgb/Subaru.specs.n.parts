...
void main() async {
  ...
  final specFiles = await specDir.list().where((e) => e.path.endsWith('.json')).toList();
          ...
          if (s['tags'] != null) {
            ...
            for (var t in tags) {
              ...
              if (cleanTag.isNotEmpty) {
                ...
                tagsToFiles.putIfAbsent(cleanTag, () => []).add(file.path.split(Platform.pathSeparator).last);
              }
            }
          }
        }
  ...
  print('(Tags found in specs that match vehicle trim words, but are NOT in knownTrims)');
